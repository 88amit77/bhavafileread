...
def gr2(json_data, graphtype):
    ...
    if graphtype == 1:  # Bollinger bands indicator
        ...
        traces1.append(plotly.graph_objs.Scatter(x=df.DateTime, y=df.UpperBand,
                                                 fill=None, line=dict(color='rgb(143, 19, 13)'), name='UB',
                                                 mode='lines'))
        traces1.append(plotly.graph_objs.Scatter(x=df.DateTime, y=df.LowerBand,
                                                 fill='tonexty', line=dict(color='rgb(143, 19, 13)'), name='LB',
                                                 mode='lines'))
        ...
        traces4.append(
            plotly.graph_objs.Scatter(x=df.DateTime, y=df['senkou_span_a'], fill='none', name='senkou_span_a',
                                      mode='lines'))
        traces4.append(
            plotly.graph_objs.Scatter(x=df.DateTime, y=df['senkou_span_b'], fill='tonexty', name='senkou_span_b',
                                      mode='lines'))
