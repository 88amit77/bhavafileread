...
for i in range(1, diff.days + 1):
    ...
    if a.status_code == 200:
        ...
        deliverable = requests.get(
            'https://nseindia.com/archives/equities/mto/MTO_' + d + m + y + '.DAT').text.splitlines()
        del deliverable[:4]
        ...
        if len(indices) > 300:  # sometimes nse doesnt give the index file, so the if condition
            ...
            indx[['Index Name', 'Index Date', 'Open Index Value', 'High Index Value', 'Low Index Value',
            ...
            indx = indx.rename(columns={'Index Name': 'SYMBOL', 'Index Date': 'TIMESTAMP', 'Open Index Value': 'OPEN',
                                        ...
                                        'Closing Index Value': 'CLOSE', 'Volume': 'TOTTRDQTY'})
